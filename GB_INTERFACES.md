...
> **STATUS: IN FORCE as of 2026-09-02; supersedes all prior versions; changes
> require both humans.**
>
> Signed 2026-09-02 by **teakeycee** and **Jhoosier** (see Change log). This file
> replaced the pre-sign-off frozen seam; `GB_INTERFACES.SIGNOFF-DRAFT.md` was
> swapped in here and deleted. This is the single file of record.
>
> **RULE:** a field changes HERE FIRST, **by both humans' agreement**, before any
> code depends on it. Items marked **`DECIDED (2026-09-02)`** record a sign-off
> outcome; the positions behind them are in `docs/SIGNOFF_REVIEW_teakeycee.md`,
> `docs/SIGNOFF_REVIEW_jhoosier.md`, `docs/F2_wire_check.md`, and
> `docs/signoff_agenda.md`.
...
"Tiki / TKC" labels are stale and are retired.
...
was previously undefined here and lived only in `tests/conftest.py`. It is lifted
in here.
...
- 2026-09-02 — **SWAP, by human order (teakeycee).** The signed draft was swapped
  in over `GB_INTERFACES.md`; `GB_INTERFACES.SIGNOFF-DRAFT.md` is deleted. Banner
  changed from DRAFT to **IN FORCE**. `CLAUDE.md` build rules aligned (pinned core
  `checks[]` vocabulary; this file named as the seam of record). No shape content
  changed in this step — swap and banner only.
